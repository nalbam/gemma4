...
`webapp.py`·`chat.py`·`templates/`(UI)는 두 백엔드가 공유하며 OS에 맞는 엔드포인트를 자동으로 향한다. `GEMMA_BACKEND=mlx|ollama`로 강제할 수 있다.

## 환경

- **macOS** (Apple Silicon) — MLX, 모델 [`mlx-community/gemma-4-12B-it-4bit`](https://huggingface.co/mlx-community/gemma-4-12B-it-4bit) (~7GB, 추론 시 ~11GB)
...
RTX 4070 SUPER(12GB) 기준 4bit(~7GB)는 전부 VRAM에 적재된다.
...
```bash
./scripts/start.sh         # 4bit 백그라운드 기동 + 준비까지 대기 (기본)
...
```bash
# 포그라운드 (Ctrl-C로 중지)
python -m mlx_vlm.server --model mlx-community/gemma-4-12B-it-4bit --host 127.0.0.1 --port 8080

# 백그라운드 (터미널 닫아도 유지)
nohup python -m mlx_vlm.server --model mlx-community/gemma-4-12B-it-4bit --host 127.0.0.1 --port 8080 > /tmp/gemma4_server.log 2>&1 &
```

아래 로그가 뜨면 준비 완료 — 엔드포인트 `http://127.0.0.1:8080/v1`:
...
pkill -f mlx_vlm.server                                                   # 중지 (포그라운드는 Ctrl-C)
```

### WSL (Ollama)

Ollama 데몬(systemd)이 서빙을 담당한다. 모델은 첫 요청 또는 웹 UI **"시작"**으로 VRAM에 적재된다 — 엔드포인트 `http://127.0.0.1:11434/v1`.
...
python chat.py                          # 대화형 (멀티턴, 종료: exit / quit / Ctrl-D)
```
...
# WSL: 호스트를 :11434, model 을 "gemma4:12b" 로 바꾼다
```
...
r = client.chat.completions.create(
    model="mlx-community/gemma-4-12B-it-4bit",   # WSL: "gemma4:12b"
...
웹 UI 또는 `./scripts/monitor.sh [간격초]`(기본 2초)로 전체 시스템 + 가속기 자원을 막대그래프로 본다.

### macOS (MLX)

- **시스템** — CPU는 `top` user+sys(전체 코어 기준), MEM은 시스템 물리 메모리.
- **gemma4 프로세스** — CPU는 raw %(추론은 GPU 위주라 보조적으로만 오른다). MEM은 `footprint`(phys_footprint)로 모델 **실제 점유량**을 표시한다(`ps` RSS는 MLX/Metal 메모리를 못 잡아 부정확). 추론해도 거의 안 변하는 게 정상 — 모델은 로드 시점에 메모리를 점유하고, 추론은 GPU 연산이라 증감이 없다(KV cache만 미세 증가).
- MLX 추론의 실제 GPU 부하는 별도로(sudo 필요): `sudo powermetrics --samplers gpu_power -i 1000 -n 1`

### WSL (GPU)

- "gemma4" 패널이 `nvidia-smi` 기반 **GPU(util·VRAM)**를 표시한다.
